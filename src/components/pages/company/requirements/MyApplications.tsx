...
const MyApplications = () => {
  ...
  useEffect(() => {
    ...
    const filtered = jobDataOrg.filter((item) => {
      // Check client filter
      const clientMatch = searchFilter.client === '' || item.client === searchFilter.client;
      // Check status filter
      const statusMatch = searchFilter.status === '' || item.status === searchFilter.status;
      // Check requirement type filter
      const requirementTypeMatch =
        searchFilter.requirementType === '' || item.requirementType === searchFilter.requirementType;
      // Check search input
      const searchMatch = searchFilter.searchValue === "" || item.role.toLowerCase().includes(searchFilter.searchValue.toLowerCase());
    ...
    <div className="px-2 py-3 h-full">
      <div className="flex flex-row gap-2 mb-3">
        <TextField
          ...
          onChange={(event) => {            
            setSearchFilter({...searchFilter, searchValue:event.target.value});
          }}
          placeholder="Search"
          slotProps={{
            input: {
        ...
        <FormControl  size="small">
          ...
          <Select
            ...
            onChange={(event) => {
              setSearchFilter((prev:any)=>({...prev, client:event.target.value}))
            }}
          >
        ...
        <FormControl  size="small">
          ...
          <Select
            ...
            onChange={(event) => {
              setSearchFilter((prev:any)=>({...prev, status:event.target.value}))
            }}
          >
            <MenuItem value={''}>All</MenuItem>
        ...
        <FormControl  size="small">
          ...
          <Select
            ...
            onChange={(event) => {
              setSearchFilter((prev:any)=>({...prev, requirementType:event.target.value}))
            }}
          >
            <MenuItem value={''}>All</MenuItem>
      ...
      <div className="table-body">
        <table>
          ...
          {jobData.map((job, index) => (
            <tr className="cursor-pointer" key={index} onClick={() => handleRowClick(job.id)}>
