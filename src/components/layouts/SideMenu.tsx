  ...
  return <>
      ...
      padding: 0
        ...
        <div className="flex w-full p-1 pl-2 gap-1 items-center rounded-md hover:bg-primary-hover text-secondary-text">
          <ListItemIcon className={`!min-w-[unset] !w-[16px] ${false ? "!text-primary" : "!text-secondary-text"}`}>
            {item.icon}
          </ListItemIcon>
      ...
      <Collapse in={isOpen} timeout="auto">
        ...
        "& .MuiListItemButton-root": {
          ...
          {item.child?.map((route: any, index: number) => <SidebarItem route={route} key={route.text} />)}
        </List>
      </Collapse>
    </>;
};
...
const SidebarItem: React.FC<SidebarItemProps> = ({
  ...
  return <ListItemButton sx={{
    padding: 0
      ...
      isActive
    }) => `flex w-full p-1 pl-2 gap-1 items-center rounded-md hover:bg-primary-hover ${isActive ? "bg-primary-hover text-primary" : "text-secondary-text"}`}>
        {({
        isActive
      }) => <>
            <ListItemIcon className={`!min-w-[unset] !w-[16px] ${isActive ? "!text-primary" : "!text-secondary-text"}`}>
              {route.icon}
            </ListItemIcon>
            <ListItemText primary={route.text} classes={{
          primary: "!text-base"
        }} />
          </>}
      </NavLink>
